...
# =============================================================================
# GitHub Copilot Custom Agents Setup Script (IntelliJ / JetBrains IDEs)
# =============================================================================
# Instala os agentes customizados para uso no GitHub Copilot Chat em IDEs
# JetBrains (IntelliJ IDEA, WebStorm, PyCharm, GoLand, etc.).
#
# Locais de instalação:
#   --project   .github/agents/          (visível no workspace)
#   --global    ~/.copilot/agents/        (visível em todos os projetos)
#              fallback legado: ~/.claude/agents/
#
# Ref: https://www.jetbrains.com/help/idea/github-copilot.html
# =============================================================================
...
uninstall_global() {
    ...
    if [ ! -d "$dest" ] && [ -d "$HOME/.claude/agents" ]; then
        print_warn "Usando caminho legado: ~/.claude/agents/"
...
show_menu() {
    print_header "GitHub Copilot Custom Agents Setup (IntelliJ / JetBrains)"
    ...
    echo "  4) Remover Globalmente        (~/.copilot/agents/)"
    ...
    if [ $# -eq 0 ]; then
        ...
        case "$1" in
            --project)
                ...
                install_project
                echo ""
                print_info "Reinicie a IDE para ativar: File > Invalidate Caches... > Restart"
                ;;
            --global)
                select_agents
                install_global
            ...
            --help|-h)
                ...
                echo "  --uninstall-global   Remove agentes de ~/.copilot/agents/"
